#!/usr/bin/env bash
# pre-install.sh — Moodle pre-install hook.
...
[[ -n "${_MOODLE_PRE_INSTALL_DONE:-}" ]] && return 0
...
ssl_full_setup "moodle" "PARAM_MOODLE_HOSTNAME" "moodle" 80 "moodle-tls"

export _MOODLE_PRE_INSTALL_DONE=1
