#!/usr/bin/env bash
# pre-install.sh — Matomo pre-install hook.
...
[[ -n "${_MATOMO_PRE_INSTALL_DONE:-}" ]] && return 0
...
ssl_full_setup "matomo" "PARAM_MATOMO_HOSTNAME" "matomo" 80 "matomo-tls"

export _MATOMO_PRE_INSTALL_DONE=1
