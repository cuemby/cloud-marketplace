#!/usr/bin/env bash
# pre-install.sh — Grafana pre-install hook.
...
[[ -n "${_GRAFANA_PRE_INSTALL_DONE:-}" ]] && return 0
...
ssl_full_setup "grafana" "PARAM_GRAFANA_HOSTNAME" "grafana" 3000 "grafana-tls"

export _GRAFANA_PRE_INSTALL_DONE=1
